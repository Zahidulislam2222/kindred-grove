 ...
 * using central client timing so rapid changes (e.g. dragging price inputs) coalesce.
 */
(function () {
  'use strict';

  const form = document.querySelector('[data-kg-filter-form]');
  if (!form || !window.KGClient?.configValid) return;

  let debounceTimer = null;

  function submitSoon() {
    clearTimeout(debounceTimer);
    debounceTimer = setTimeout(() => form.submit(), window.KGClient.limits.collectionFilterDebounceMs);
  }
